  ...
  return (
    ...
    >
      <div className="flex flex-col h-full">
        <div className={`flex items-center justify-between h-16 bg-indigo-900 ${isExpanded ? 'px-4' : 'px-2'}`}>
          <img src="/logo.png" alt="IMD-Care Logo" className="h-8 w-auto" />
          {isExpanded && <span className="ml-2 text-xl font-semibold hidden lg:inline">IMD-Care</span>}
          <button
            ...
            {navItems.map((item) => (
              <li key={item.name} className="mb-2">
                ...
                >
                  ...
                  <span className={`ml-3 transition-opacity duration-300 ${isExpanded ? 'opacity-100' : 'opacity-0 lg:w-0'}`}>
                    {item.name}
                  </span>
                </Link>
              </li>
